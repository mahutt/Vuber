    ...
    <div className="w-full max-w-lg mx-auto bg-white m-10 p-8 rounded-lg shadow-md">
      ...
      <div className="space-y-4">
        <div className="text-center text-gray-800 font-medium">
          ...
          <p className="text-xl font-extrabold text-blue-600 tracking-tight">{order.id}</p>
        </div>

        <div className="border-t border-gray-200 my-6"></div>

        <div className="text-gray-700">
          <h3 className="text-2xl font-extrabold tracking-tight mb-2">Order Details:</h3>

          <p><h3 className="text-xl font-extrabold tracking-tight mb-2">Total:</h3></p>

          <p><p className="font-extrabold tracking-tight mt-2">{"$"+order.total.toFixed(2)+" CAD"}</p></p>
          ...
          <ul className="list-disc ml-5">
            {order.parcels.map((parcel, index) => (
              ...
              <span className="font-semibold">{parcel.name || `Parcel ${index + 1}`}</span>
              <p className="text-sm text-gray-600">{parcel.description || "No description available"}</p>
            </li>
            ))}
          </ul>
          <p className="font-extrabold tracking-tight mt-2">Pick up Instructions:</p>
          <p>{order.pickupInstructions}</p>

          <p className="font-extrabold tracking-tight mt-2">Drop off Instructions:</p>
