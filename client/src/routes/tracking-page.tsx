...
function Tracking() {
    ...
    const track = async() =>{
      ...
      if(response.status == 200){
        ...
        var currLoc = await fetchPlaceName(response.data.newCurrLocation.xCoord,response.data.newCurrLocation.yCoord)
        if(currLoc == null){currLoc = ''}
        setCurrentLocation(currLoc);

        var dest = await fetchPlaceName(response.data.destinationCoords.xCoord,response.data.destinationCoords.yCoord)
        if(dest == null){dest = ''}
        setEndLocation(dest);
        ...
        for(let i =0; i < prevCoordArr.length; i++){
           ...
           let locString = await fetchPlaceName(prevCoordArr[i].xCoord,prevCoordArr[i].yCoord)
           if(locString){
        ...
        <section className="w-[1000px] mx-auto mb-4 overflow-y-auto">
          ...
          <div>
            ...
            <Table>
              ...
              <TableBody>
                {previousLocations.slice().reverse().map((location, index) => (
                  <TableRow key={index}>
